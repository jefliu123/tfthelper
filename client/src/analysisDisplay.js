	...
	render() {
		...
		if (this.props.parentResponse.champions) {
			...
			for (var i = 0; i < 4; i++) {
				...
				champDisplayArray.push(
					<ChampDisplay
						champName={championsArray[i][0]}
						champInfoObject={championsArray[i][1]}
					/>
				);
			}
		}
		...
		return (
			<div className="Champion-body">
				<div className="Champion-header">
					...
					<p>
						Preferred Carries:<br></br>(Past 10 Games)
					</p>
				</div>
				<div className="Champion-row">{champDisplayArray}</div>
				<div className="Analysis-box">
					<p>
						Verdict:
						{abuser && (
							<span className="red">
								&nbsp;{mostUsedCarry.substring(5)} one-trick. Yikes
							</span>
						)}
						{!abuser && (
							<span className="green">&nbsp;Pretty flexible. Not bad</span>
						)}
					</p>
				</div>
			</div>
